...
// *** Create Record ***
// required options: base, table, record

module.exports.createRecord = async function (options) {
    const base = new Airtable({apiKey: process.env.AIRTABLE_API_KEY}).base(options.baseId);
    let theRecord = await base(options.table).create(options.record)
        .then(data => {
            console.log(`saved to airtable:\n${JSON.stringify(data, null, 4)}`); 
            return data
        })
        .catch(err => {console.log("there was an error:" + err); return err })
    return theRecord;
}
